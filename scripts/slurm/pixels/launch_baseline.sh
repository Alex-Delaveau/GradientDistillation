...
# Same seeds, IPC and evaluation protocol as the pixels experiments.
#
# Usage:
#   DATASET=fishnet20 ./launch_baseline.sh                     # h100, 3 ipc x 5 seeds = 15 jobs
#   DATASET=fishnet20 ARCH=a100 ./launch_baseline.sh           # same grid on A100-80G
#   DATASET=f4k ARCH=a100 DRYRUN=1 ./launch_baseline.sh        # preview, submit nothing
#   DATASET=fishnet20 IPC="1" SEEDS="3407" ./launch_baseline.sh    # single smoke run
#   DATASET=fishnet20 TIME=06:00:00 ./launch_baseline.sh       # same walltime for all jobs -> backfill
#   DATASET=foo DATA_ROOT=$WORK/datasets/foo ./launch_baseline.sh  # dataset not in the table
set -euo pipefail

SLURM_SCRIPT="$(dirname "$0")/distill_baseline.slurm"

DATASET=${DATASET:?DATASET must be set (f4k | fishnet20 | aqua20 | ...)}

# --- data_root per dataset: each loader has its own path convention ---
#   f4k       : data_root = dataset directory itself
#   fishnet20 : data_root = parent directory (loader appends FishNet20/)
#   aqua20    : data_root = dataset directory itself
case "$DATASET" in
  f4k)       DATA_ROOT_DEF="$WORK/datasets/F4K" ;;
  fishnet20) DATA_ROOT_DEF="$WORK/datasets" ;;
  aqua20)    DATA_ROOT_DEF="$WORK/datasets/aqua20" ;;
  *)         DATA_ROOT_DEF="" ;;
esac
DATA_ROOT=${DATA_ROOT:-$DATA_ROOT_DEF}
if [ -z "$DATA_ROOT" ]; then
    echo "no default DATA_ROOT for DATASET='$DATASET' -> set DATA_ROOT explicitly" >&2
    exit 1
fi
if [ ! -d "$DATA_ROOT" ]; then
...
OUTPUT_ROOT=${OUTPUT_ROOT:-$WORK/projects/GradientDistillation/output/baseline_${DATASET}}
...
if [ -n "$TIME" ]; then
    echo "walltime override: $TIME"
else
    echo "walltime: ipc1=$TIME_IPC1 ipc3=$TIME_IPC3 ipc5=$TIME_IPC5"
fi
echo "ipc($IPC) x seeds($SEEDS) -> $njobs jobs $([ "$DRYRUN" = "1" ] && echo '(DRYRUN)' || echo 'submitted')"
