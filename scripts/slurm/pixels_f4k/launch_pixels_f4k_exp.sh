#!/bin/bash
# Pixel-space distillation experiment (none / slurpp / ppg), n=5 seeds x 3 IPC. Using Fish4Knowledge dataset
#
# Usage:
#   ./launch_pixels_f4k_exp.sh                      # 3 priors x 3 ipc x 5 seeds = 45 jobs
#   DRYRUN=1 ./launch_pixels_f4k_exp.sh             # preview, submit nothing
#   IPC="1" ./launch_pixels_f4k_exp.sh              # one ipc only (15 jobs)
#   PRIORS="slurpp" ./launch_pixels_f4k_exp.sh      # one prior only
#   SEEDS="3407" IPC="1 5" ./launch_pixels_f4k_exp.sh
#   ONLY=ppg ./launch_pixels_f4k_exp.sh             # substring filter on run_name
set -euo pipefail

SLURM_SCRIPT="$(dirname "$0")/distill_pixels_f4k.slurm"

OUTPUT_ROOT="$WORK/projects/GradientDistillation/output/pixels_f4k"
