...
def recursive_load_config(config_path: str) -> OmegaConf:
    ...
    if base_configs is not None:
        ...
        for _path in base_configs:
            # résout les chemins relatifs depuis le dossier du yaml courant,
            # pas depuis le cwd -> portable (PC / Jean Zay, peu importe le cwd)
            if not os.path.isabs(_path):
                _path = os.path.join(base_dir, _path)
            assert (
                os.path.abspath(_path) != os.path.abspath(config_path)
            ), "Circular merging, base_config should not include itself."
