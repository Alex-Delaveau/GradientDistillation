    ...
    def __call__(self,
        ...
        **kwargs,):
        ...
        hidden_states1 = F.scaled_dot_product_attention(
            q1.transpose(1, 2), k1.transpose(1, 2), v1.transpose(1, 2), scale=attn1.scale
        ).transpose(1, 2)
        hidden_states2 = F.scaled_dot_product_attention(
            q2.transpose(1, 2), k2.transpose(1, 2), v2.transpose(1, 2), scale=attn2.scale
        ).transpose(1, 2)

        hidden_states1_cross = F.scaled_dot_product_attention(
            q1_cross.transpose(1, 2), k2.transpose(1, 2), v2.transpose(1, 2), scale=attn2.scale
        ).transpose(1, 2)
        hidden_states2_cross = F.scaled_dot_product_attention(
            q2_cross.transpose(1, 2), k1.transpose(1, 2), v1.transpose(1, 2), scale=attn1.scale
        ).transpose(1, 2)
